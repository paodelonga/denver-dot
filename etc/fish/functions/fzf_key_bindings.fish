...
function fzf_key_bindings
    ...
    function fzf-repos-widget -d 'Fuzzy Repository'
        if not test -z "$REPOS_PATH"
            ...
            set _FZF_OUTPUT (fdfind . "$REPOS_PATH" --type directory --hidden --follow --prune --no-ignore-vcs -E 'node_modules' -E '.git' | sed 's/.*\///' | fzf --print-query --border-label 'Fuzzy Repository' -0 | tail -1)
    ...
    function fzf-editor-widget --description "Fuzzy Editor"
        set _FZF_OUTPUT (fdfind --type file --follow --hidden --no-ignore-vcs -E 'node_modules' -E '.git' | fzf --border-label 'Fuzzy Editor' -0)
